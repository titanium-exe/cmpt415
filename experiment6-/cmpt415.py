...
def generate_basic_p4_prompt(high_level_prompt):
    detailed_prompt = (
        f"{high_level_prompt.strip()}. "
        "Write complete and valid P4_16 code that compiles with `p4c-bm2-ss` for the BMv2 `v1model` architecture. "
        "Include definitions for Ethernet and IPv4 headers, a parser for both, and empty `metadata` struct. "
        "Implement the ingress pipeline using `standard_metadata.egress_spec` to forward or drop packets. "
        "Use control blocks: `MyParser`, `MyIngress`, `MyEgress`, `MyVerifyChecksum`, `MyComputeChecksum`, and `MyDeparser`. "
        "End with: `V1Switch(MyParser(), MyVerifyChecksum(), MyIngress(), MyEgress(), MyComputeChecksum(), MyDeparser()) main;`"
    )
